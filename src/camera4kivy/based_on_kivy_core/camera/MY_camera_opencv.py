...
class CameraOpenCV(CameraBase):
    ...
    def __init__(self, **kwargs):    #####aspect_ratio = '4:3', **kwargs):
        ...
        self._device = None
        '''
        self.resolution_not_set = kwargs.get('resolution') == []
        self.aspect_ratio = aspect_ratio
        self.res169 = [[15360, 8640],
                       [8192, 4608],
                       [7680, 4320],
                       [6016, 3384],
                       [5120, 2880],
    ...
    def init_camera(self):
        # Override the CameraBase settings
        '''
        if self.resolution_not_set:
            self._resolution = [6400, 4800]
        '''
        ...
        if self.opencvMajorVersion == 1:
            ...
            if self.aspect_ratio == "16:9":
                # the 16:9 resolution will be lower than the 4:3 resolution
                width = self._device.get(cv2.CAP_PROP_FRAME_WIDTH)
                height = self._device.get(cv2.CAP_PROP_FRAME_HEIGHT)
                width169 = self.res169[-1][0]
                height169 = self.res169[-1][1]
                for r in self.res169:
                    if width >= r[0]:
                        width169  = r[0]
                        height169 = r[1]
                        break
                self._device.set(cv2.CAP_PROP_FRAME_WIDTH, width169)
                self._device.set(cv2.CAP_PROP_FRAME_HEIGHT, height169)  
            '''
